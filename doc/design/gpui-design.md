...
挂在**用户消息**上的低频操作入口（用户消息 = 一轮对话的起点）。当前唯一动作是**编辑**（编辑历史用户消息 = 从该轮分支，见下「编辑即 fork」）。

- **克制第一**：默认态**极弱可见**（图标 `text_disabled`、初始透明），仅当该轮被 hover 时提亮（`group_hover` 提升一级）。
- **纯图标无可见文字**；hover 背景只提升一级（surface lift + hairline）。
- **不碰 accent**（accent 只留给主操作）。图标用原创描边字形，不借用现成图标集（§5）。
- **首轮例外**：第一条用户消息不渲染编辑入口（空上下文 = 等同 new session，无意义）。

**编辑即 fork（edit-as-fork）**：把一条历史用户消息载入输入框即进入**编辑态**（输入框带标记「编辑第 N 轮」+ Esc/清除退出）。编辑态的发送入口与正常发送有**视觉区分**（分支意味的字形变体 `⤦` vs 正常 `→`，色调仍走 token），但**不出现 "fork" 字样**。语义（已定稿）：
- 编辑 = 分支。编辑态下**只要发送就 fork**：`fork_session(parent, 该轮seq)` 建分支 → 把输入框文本发进新分支 → 切换到新分支。**不做逐字 diff**（没有「改没改」的灰色地带）。
- 原 session 的 append-only log **不被改写**（编辑永远开新分支，原消息原样保留）。
- 这是惰性 fork 的落地：编辑动作本身不建会话，发送那一刻才真正 fork + 发送。
