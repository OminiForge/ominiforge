...
> **新架构定位**：Session 在新架构下由 `ofg-def-agent` 的 SessionStore（append-only 事件溯源）承载，fork/compaction 语义不变。组合运行时与拓展机制见 [`runtime-architecture.md`](./runtime-architecture.md)。
...
Session 的退役分两级——**archive（归档，安全、日常）** 与 **hard delete（硬删除，危险、罕用）**——两者都是沙箱 `release` 的触发点（见 `runtime-architecture.md` §5：session 被退役才 release，不绑 thread）。
