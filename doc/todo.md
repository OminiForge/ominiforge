...
代码：todo 状态与操作见 [`src/agent/todo.rs`](../src/agent/todo.rs)（`TodoItem`、`StepStatus`、
`TodoOp`、`apply_todo_op`、`render`、`descriptor`）；`SessionRuntime` / `TurnState` 门类、
完成度门、卡死检测见 [`src/agent/mod.rs`](../src/agent/mod.rs)。本文讲设计意图与边界，结构体
与控制逻辑细节以代码及其注释为准。

## 1. 定位与边界

Todo 是 **turn 内**的工作清单机制，把一个较长目标拆成有序步骤，让 agent 在多 round 推进中
不迷失方向。它属于架构 `Core Agent Layer` 中 `planning / execution policy` 的落地。名字刻意
叫 `todo` 而非 `plan`：它只是一份执行中的打勾清单，"规划"这个名字留给后续更大的系统。

**必须和 Phase 4 的 Task 系统切清楚：**

| 维度 | Todo（本文） | Task（Phase 4） |
|------|--------------|-----------------|
...
Todo 不负责跨 context 的拆分。当任务大到单个 context 装不下时，那是 subagent（Phase 4/5）的
边界，不是 Todo 的职责。

## 2. 设计原则

- Todo tool 只管理清单状态，**不执行任何动作**（不读文件、不跑命令）。planning 与 doing 分离。
- Todo 状态权威副本存于 `SessionRuntime`（内存，跨 turn 存活），事实记录仍是 events.jsonl 中
...
| **session-scoped** | **跨 turn，会话级** | **`SessionRuntime`（调用方持有）** | **context（对话视图）、todo、ledger** |
...
关键判断：**todo 是 session-scoped，不是 turn-scoped**。一个 `blocked` 步骤的本质是"挂起、
等用户介入"，天然跨 turn——用户下个 turn 补充信息时，agent 必须仍记得清单卡在哪、为什么。
若随 turn 销毁，blocked 状态就失去意义。因此 todo 与 context 同级放进 `SessionRuntime`；只有
...
单个 built-in tool（名 `todo`），只有两种调用形态（schema 为 `oneOf` 两分支，与其他叶子
工具同风格）：`{"op": "init", "steps": [...]}` 建立清单；`{"ops": [...]}` 变更清单，
`ops` 数组按序应用多个 leaf op（`start` / `complete` / `cancel`(reason) / `block`(reason) /
`add`(after_id?)），遇错即停、之前的 op 保留，单个变更就是单元素数组——改多步不用发多个
并行 todo 调用。`init` 不能出现在 `ops` 里（`LeafOp` 枚举不含此变体，反序列化直接拒绝，
...
`todo` 是首个**操作 agent 自身状态**的控制工具，read/write/shell 是纯 I/O 叶子工具。叶子
工具的 `Tool::invoke` 拿不到 TurnState（也不应该）。落地方式：

- `todo` 不进 `ToolRegistry`（registry 只放叶子工具）。其 descriptor 由 `TurnState` 在组装
  tool schemas 时直接贡献，与叶子工具一同广播给模型。
- agent loop 在 dispatch 时**按工具名拦截** `todo`：不走 `tool.invoke()`，改调 `apply_todo_op`
  把变更写进 `TurnState.todo`，再渲染当前清单回灌。
- 事件一致性：todo 调用照常用 `ToolEvent::Started` / `Completed` 包裹（`ToolSource::Builtin`），
...
behavioral guidance 写进 `todo` tool 的 `description`（英文），不进 profile system prompt：
复杂任务先 `init` 再逐步推进，琐碎单步任务无需建清单；每步开始 `start`、完成 `complete`；
仅客观不可达时 `cancel`、需用户介入时 `block`（均须具体说明）；禁止因困难而 cancel/block；
所有步骤须到达终态方可结束任务。

## 10. 多 turn 行为与恢复

todo 是 session-scoped，跨 turn 存活。turn 结束只销毁 `TurnState`（turn_id、round、计数器），
todo 与 context 留在 `SessionRuntime`。

- **blocked 步骤跨 turn 延续**（todo 必须跨 turn 的核心理由）：turn 因 `blocked` 干净结束并把
  原因带给前端。下个 turn 模型同时看到用户新输入 + 当前清单状态，自行判断：用户在回应
  blocked → 把对应步骤 `start` 改回继续；用户开了新话题 → `init` 覆盖或 `cancel` 旧步骤。
  完成度门只看非终态，`blocked` 是终态不触发门——所以"挂起等用户"能干净交还控制权又不丢状态。
- **陈旧清单不纠缠新话题**：全终态的清单不触发门，模型在新 turn 可无视或 `init` 覆盖。
  模型始终能看到清单现状并自主决定去留，不被旧清单绑架。
- **恢复（resume）**：`SessionRuntime` 是内存结构，进程退出即失，但每个 todo op 都已写入
...
- 中途 nag（连续 N round 没碰清单就提醒）：完成度门 + 卡死检测已覆盖防跑偏诉求。
- 完成度驱动的 verification nudge（"快做完了，该自检/跑测试"）：留待接入 reviewer 阶段。
- first-class `TodoEvent`：`ToolEvent::Started.input` 已落完整 todo 操作，replay/monitor 可还原。
- 跨 turn 权威 todo 状态。
