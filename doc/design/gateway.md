...
**精确类型与签名以代码为准**：配置见 [`src/gateway/config.rs`](../crates/ominiforge/src/gateway/config.rs)；
session actor 见 [`src/gateway/actor.rs`](../crates/ominiforge/src/gateway/actor.rs)；registry 见
[`src/gateway/registry.rs`](../crates/ominiforge/src/gateway/registry.rs)；HTTP/SSE/WS server 见
[`src/gateway/server.rs`](../crates/ominiforge/src/gateway/server.rs)。本文只讲设计意图与契约。
...
完整路由与请求/响应以 [`src/gateway/server.rs`](../crates/ominiforge/src/gateway/server.rs) 为准。
