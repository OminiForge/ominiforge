...
> **新架构定位**：新架构下统一为「容器内经 direnv 准备环境」，见 runtime-architecture.md §5.5；本文为机制细节。组合运行时与拓展机制见 [`runtime-architecture.md`](./runtime-architecture.md)。
...
快照与 workspace 配置同生命周期：`DELETE /workspaces/config/{id}` 在删 `<id>.toml` 时一并删除快照；孤儿处理沿用显式 GC 模型（见 `runtime-architecture.md` §7）。

### 21.5 已知局限

- **boxlite 沙箱后端不应用 env overlay**：overlay 值是宿主路径（如 `/nix/store/...`），guest 里没有挂载点了无意义——待执行世界（见 `runtime-architecture.md` §5）的 `/nix/store` 挂载设计落地后再接。passthrough（默认后端）正常。
